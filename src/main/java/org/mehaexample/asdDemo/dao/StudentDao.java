...
//	}
	...
	public List<Student> searchStudentRecord(String firstName) {
		org.hibernate.query.Query query = session.createQuery("from Student where firstName like :studentfirstName");
		firstName = firstName+'%';
	...
	// added search student record using first name
	public List<Student> searchSimilarStudents(String degree) {
		DegreeCandidacy degreeType = DegreeCandidacy.valueOf(degree.toUpperCase());
		org.hibernate.query.Query query = session.createQuery("from Student where degreeCandidacy = :degree");
		query.setParameter("degree", degreeType);
		List<Student> list = query.list();
		return list;
	}
